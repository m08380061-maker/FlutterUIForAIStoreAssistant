...
class _PhotoEnrollmentScreenState extends State<PhotoEnrollmentScreen> {
  ...
  bool get _hasMissingFields {
    return _categoryCtrl.text.trim().isEmpty ||
        _purchasePriceCtrl.text.trim().isEmpty ||
        _sellingPriceCtrl.text.trim().isEmpty;
  }
  ...
  Future<void> _save() async {
    ...
    if (_nameCtrl.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.tr.productNameRequired), backgroundColor: AppColors.error),
      );
      return;
    }
    ...
    try {
      if (_createAsDraft) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(context.tr.draftCreated), backgroundColor: AppColors.warning),
          );
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
                ...
                child: Container(
                  ...
                  child: _capturedImagePath != null
                      ...
                      : Column(
                          ...
                          children: [
                            Icon(Icons.camera_alt_rounded, size: 48, color: AppColors.primary),
                            const SizedBox(height: 8),
                            Text(tr.tapToCapture, style: Theme.of(context).textTheme.bodyMedium),
                          ],
                        ),
                ),
              ),
              ...
              OutlinedButton.icon(
                onPressed: _isSearching ? null : _enrichFromName,
                icon: const Icon(Icons.search_rounded),
                label: Text(tr.searchWebImages),
              ),
              const SizedBox(height: 12),

              if (_hasMissingFields && _nameCtrl.text.isNotEmpty)
                Padding(
              ...
              CustomButton(
                label: _hasMissingFields ? tr.saveAsDraft : tr.saveProduct,
