...
class _InvoiceEnrollmentScreenState extends State<InvoiceEnrollmentScreen> {
          ...
          children: [
            GestureDetector(
              ...
              child: Container(
                ...
                child: _invoiceImagePath != null
                    ? ClipRRect(
                        borderRadius: BorderRadius.circular(AppConstants.radiusMedium),
                        child: Image.file(File(_invoiceImagePath!), fit: BoxFit.cover),
                      )
                    : Column(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(Icons.receipt_long_rounded, size: 48, color: AppColors.accentOrange),
                          const SizedBox(height: 8),
                          Text(tr.captureInvoice, style: Theme.of(context).textTheme.bodyMedium),
                        ],
                      ),
              ),
            ),
            ...
            if (_isProcessing) ...[
              Center(
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text(tr.processingInvoice, style: Theme.of(context).textTheme.bodyMedium),
                  ],
                ),
              ),
              ...
              if (_ocrResult!.items.isEmpty) ...[
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 8),
                      Text(tr.noItemsExtracted, style: Theme.of(context).textTheme.bodyMedium),
                    ],
                  ),
                ),
              ] else ...[
                ...
                Text(
                  '${_ocrResult!.items.length} ${tr.itemsFound}',
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(tr.enrichingImages, style: Theme.of(context).textTheme.bodySmall),
                      ],
                    ),
                  ),

                CustomButton(
                  label: tr.createDraftsFromItems,
                  onPressed: _isEnriching ? null : _createDrafts,
  ...
  Future<void> _processOcr() async {
    ...
    try {
      ...
      setState(() {
        _ocrResult = result;
        _selectedItems = List.from(result.items);
      });
    } finally {
    ...
    try {
      for (final item in _selectedItems) {
        ...
        await ProductDraftService.instance.createDraft(
          ...
          source: DraftSource.invoice,
          sourceImagePath: _invoiceImagePath,
          extractedData: item.toJson(),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('${_selectedItems.length} ${context.tr.draftsCreated}'),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(item.name, style: textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w500)),
                  if (item.quantity != null || item.price != null) ...[
                    const SizedBox(height: 2),
                    Text(
                      [
                        if (item.quantity != null) '${tr.qty}: ${item.quantity}',
                        if (item.price != null) tr.formatCurrency(item.price!),
                      ].join(' · '),
