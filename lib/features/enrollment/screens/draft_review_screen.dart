  ...
  Future<void> _saveAsProduct() async {
    ...
    try {
      ...
      final allImages = <String>[
        if (_draft!.sourceImagePath != null) _draft!.sourceImagePath!,
        ..._enrichedImages,
      ];
  ...
  Future<void> _deleteDraft() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(context.tr.deleteDraft),
        content: Text(context.tr.deleteDraftConfirm),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text(context.tr.cancel)),
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: Text(context.tr.delete)),
        ],
      ),
    );
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              if (_draft!.sourceImagePath != null) ...[
                ClipRRect(
                  borderRadius: BorderRadius.circular(AppConstants.radiusMedium),
                  child: Image.file(
                    File(_draft!.sourceImagePath!),
              ...
              if (_draft!.source == DraftSource.invoice && _draft!.extractedData != null) ...[
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.receipt_long_rounded, color: AppColors.accentOrange, size: 20),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
                          '${tr.extractedFromInvoice}: ${_draft!.extractedData!['name']}'
                          '${_draft!.extractedData!['quantity'] != null ? ' (${_draft!.extractedData!['quantity']} ${tr.units})' : ''}',
              ...
              OutlinedButton.icon(
                onPressed: _isEnriching ? null : _enrichImages,
                icon: const Icon(Icons.search_rounded),
                label: Text(tr.searchWebImages),
              ),
              const SizedBox(height: 12),
              CustomButton(
                label: tr.saveAsProduct,
                onPressed: _isSaving ? null : _saveAsProduct,
                isLoading: _isSaving,
              ),
            ],
          ),
