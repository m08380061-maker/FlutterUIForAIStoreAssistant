...
class OcrService {
  ...
  Future<OcrResult> _processWithFreeOcrApi(Uint8List imageBytes) async {
    try {
      ...
      final request = http.MultipartRequest('POST', uri)
        ...
        ..fields['scale'] = 'true'
        ..files.add(http.MultipartFile.fromBytes('file', imageBytes, filename: 'invoice.jpg'));
      ...
      final fullText = (parsedResults[0] as Map<String, dynamic>)['ParsedText'] as String? ?? '';
  ...
  List<ExtractedItem> _extractItemsFromText(String text) {
    ...
    for (final line in lines) {
      ...
      items.add(ExtractedItem(name: namePart, quantity: quantity ?? 1, price: price));
    }
  ...
  bool _isHeaderOrFooter(String line) {
    ...
    final keywords = ['total', 'subtotal', 'tax', 'vat', 'discount', 'change', 'cash', 'receipt', 'invoice', 'date', 'time', 'thank', 'store', 'phone', 'address', 'cashier', 'transaction', 'card', 'payment', 'balance', 'qty', 'quantity', 'item', 'description', 'price', 'amount', 's/n', 'no.', 'ref', 'www', 'http', '@'];
