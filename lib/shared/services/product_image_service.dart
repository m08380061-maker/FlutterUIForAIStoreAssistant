...
class ProductImageService {
  ...
  Future<void> saveImage({
    ...
    await _db.customStatement(
      ...
      [const Uuid().v4(), productId, localPath, sourceUrl, width, height, isPrimary ? 1 : 0, DateTime.now().millisecondsSinceEpoch],
    );
  }
  ...
  Future<void> setPrimaryImage(String productId, String imageId) async {
    await _db.customStatement('UPDATE product_images SET is_primary = 0 WHERE product_id = ?', [productId]);
    await _db.customStatement('UPDATE product_images SET is_primary = 1 WHERE id = ?', [imageId]);
  }

  Future<void> deleteImage(String imageId) async {
    await _db.customStatement('DELETE FROM product_images WHERE id = ?', [imageId]);
  }

  Future<void> deleteAllForProduct(String productId) async {
    await _db.customStatement('DELETE FROM product_images WHERE product_id = ?', [productId]);
  }
}
...
class ProductImageRecord {
  ...
  const ProductImageRecord({required this.id, required this.productId, required this.localPath, this.sourceUrl, this.width, this.height, required this.isPrimary});
}
