...
class ProductIndexService {
  ...
  Future<List<IndexedProduct>> searchByFingerprint(
    ...
    for (final row in rows) {
      final embeddingJson = row.read<String>('embedding_json');
      ...
      try {
        ...
        final aHashSim = FingerprintService.instance.hammingSimilarity(query.averageHash, fp.averageHash);
        final dHashSim = FingerprintService.instance.hammingSimilarity(query.differenceHash, fp.differenceHash);
        final histSim = FingerprintService.instance.histogramSimilarity(query.colorHistogram, fp.colorHistogram);
        final score = (aHashSim * 0.35 + dHashSim * 0.35 + histSim * 0.30);
        results.add(IndexedProduct(productId: productId, imagePath: imagePath, score: score, fingerprint: fp));
      } catch (_) {
        continue;
      }
    }
  ...
  Future<List<IndexedProduct>> getAllIndexed() async {
    ...
    return rows.map((row) {
      ...
      return IndexedProduct(productId: row.read<String>('product_id'), imagePath: row.read<String>('image_path'), score: 1.0, fingerprint: fp);
    }).toList();
  }

  Future<void> removeByProductId(String productId) async {
    await _db.customStatement('DELETE FROM product_embeddings WHERE product_id = ?', [productId]);
  }

  Future<int> getIndexSize() async {
    final rows = await _db.customSelect('SELECT COUNT(*) as count FROM product_embeddings').get();
...
class IndexedProduct {
  ...
  const IndexedProduct({required this.productId, required this.imagePath, required this.score, required this.fingerprint});
}
